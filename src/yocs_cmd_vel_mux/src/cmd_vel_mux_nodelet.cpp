...
void CmdVelMuxNodelet::cmdVelCallback(const race::drive_param::ConstPtr& msg, unsigned int idx)
{
  // Reset general timer
  common_timer.stop();
  common_timer.start();
...
void CmdVelMuxNodelet::reloadConfiguration(yocs_cmd_vel_mux::reloadConfig &config, uint32_t unused_level)
{
  ...
  **********************/
  ...
  if (output_topic_name != output_name)
  {
    ...
    output_topic_pub = pnh.advertise<race::drive_param>(output_topic_name, 10);
  ...
  **********************/
  ...
  for (unsigned int i = 0; i < cmd_vel_subs.size(); i++)
  {
    ...
    {
      cmd_vel_subs[i]->subs =
          pnh.subscribe<race::drive_param>(cmd_vel_subs[i]->topic, 10, CmdVelFunctor(i, this));
